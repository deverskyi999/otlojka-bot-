...
class Database:
    ...
    # --- НОВЫЕ МЕТОДЫ ДЛЯ МУТА, АВТООТВЕТА, XO ---
...
def register_handlers(
    ...
    # 1. Мут
    @dp.message(F.text & F.text.startswith(".mute"))
    async def handle_mute(message: Message) -> None:
        ...
        # В бизнес-чате собеседник - это chat_id
        ...
        await message.answer("✅ Собеседник замучен.")
    ...
    # 3. Спам
    @dp.message(F.text & F.text.startswith(".spam"))
    ...
    # 4. Крестики-нолики
    @dp.message(F.text & (F.text == ".xo" or F.text.startswith(".xo ")))
    async def handle_xo(message: Message) -> None:
        user_id = message.from_user.id
        ...
        opponent_username = args[1].lstrip("@") if len(args) > 1 else None
        ...
        if opponent_username:
            ...
            row = db.get_user(user_id)
            if not row or not row["business_connection_id"]:
                await message.answer("❌ Бот не подключён.")
                return
    ...
    # 5. Автоответ
    @dp.message(F.text & F.text.startswith(".auto"))
    async def handle_auto_reply(message: Message) -> None:
        ...
        elif action == "set" and len(parts[1].split(maxsplit=1)) > 1:
            new_text = parts[1].split(maxsplit=1)[1]
