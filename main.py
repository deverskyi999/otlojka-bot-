...
SUBSCRIPTION_STARS_DEFAULT = 15  # используется только как значение по умолчанию, реальная цена — в bot_settings
...
WELCOME_TEXT_DEFAULT = (
    "Здравствуй! Это SiaTimeBot — бот для времени в никнейме «Телеграм», "
    "также еще несколько очень интересных функций, успей их затестить"
)
    ...
    def init_schema(self) -> None:
        with self.connect() as conn:
            ...
            migrated = conn.execute(
                "SELECT value FROM bot_settings WHERE key = 'welcome_text_v2_applied'"
            ).fetchone()
            if migrated is None:
                conn.execute(
                ...
                conn.execute(
                    "INSERT OR IGNORE INTO bot_settings (key, value) VALUES ('welcome_text_v2_applied', '1')"
                )
...
def parse_target_input(text: str) -> tuple[Optional[datetime], Optional[str]]:
    """Парсит сообщение вида '31.12.2026 20:00 Новый год' -> (datetime, 'Новый год')."""
    text = text.strip()
...
def register_handlers(
    ...
    async def handle_set_target_date_finish(message: Message, state: FSMContext) -> None:
        user_id = message.from_user.id
        await state.clear()
        dt, label = parse_target_input(message.text or "")
        if dt is None:
            await message.answer(
            ...
            return
        db.set_target_datetime(user_id, dt.isoformat(), label)
        row = db.get_user(user_id)
        tz_offset = db.get_timezone(row, settings.timezone_offset_hours)
        if row and row["enabled"] and row["business_connection_id"]:
            await clock.apply(
