...
STYLE_KEYS: list[tuple[str, str]] = [
    ("connect", "🔗 Подключить"),
    ("copy", "📋 Скопировать"),
    ("toggle_on", "▶️ Включить"),
    ("toggle_off", "⏹ Выключить"),
    ("pay_stars", "⭐️ Оплата/продление Stars"),
    ...
    ("support", "🆘 Поддержка"),
    ("support_link", "✉️ Ссылка на поддержку"),
    ("oferta", "📄 Оферта"),
...
# Текстовые эмодзи (внутри приветственного сообщения), не привязаны к кнопке.
TEXT_EMOJI_KEYS: list[tuple[str, str]] = [
    ("welcome_check", "✅ Приветствие: галочка"),
    ("opt_time", "1️⃣ Приветствие: время"),
    ("opt_date", "2️⃣ Приветствие: дата"),
    ("opt_countdown", "3️⃣ Приветствие: отсчёт"),
    ("star", "⭐️ Приветствие: звезда"),
]
...
def build_welcome_keyboard(db: Database, has_trial: bool, is_subscribed: bool) -> InlineKeyboardMarkup:
    ...
    stars_label = f"🔁 Продлить подписку — {price_stars}⭐️" if is_subscribed else f"⭐️ Оплатить подписку — {price_stars}⭐️"
...
def register_admin_handlers(dp: Dispatcher, db: Database, settings: Settings) -> None:
    ...
    def _grid(items: list[tuple[str, str]], prefix: str, marks: dict[str, str]) -> list[list[InlineKeyboardButton]]:
        rows: list[list[InlineKeyboardButton]] = []
        pair: list[InlineKeyboardButton] = []
        for key, label in items:
            mark = marks.get(key, "")
            btn = InlineKeyboardButton(text=f"{mark}{label}", callback_data=f"{prefix}:{key}")
            pair.append(btn)
            if len(pair) == 2:
                rows.append(pair)
                pair = []
    ...
    async def handle_admin_emoji(callback: CallbackQuery) -> None:
        ...
        marks = {k: "✅ " for k, _ in _emoji_pick_keys() if k in set_keys}
        kb_rows = _grid(_emoji_pick_keys(), "pick_emoji", marks)
        kb_rows.append([InlineKeyboardButton(text="◀️ Назад", callback_data="admin_menu")])
        await callback.message.edit_text(
            "<b>🎨 Премиум-эмодзи</b>\n\n"
            "Нажмите на кнопку, для которой хотите поставить эмодзи (✅ — уже задано), "
    ...
    async def handle_pick_emoji(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=build_back_keyboard("admin_emoji"),
        )
        await callback.answer()
    ...
    async def handle_emoji_value_input(message: Message, state: FSMContext) -> None:
        ...
        await state.clear()
        await message.answer(
            "✅ Эмодзи обновлено.", reply_markup=build_back_keyboard("admin_emoji")
        )
    ...
    async def handle_admin_style(callback: CallbackQuery) -> None:
        if not _is_owner(callback.from_user.id, settings):
            ...
            return
        marks = {}
        for key, _ in STYLE_KEYS:
            style = db.get_button_style(key)
            marks[key] = {"primary": "🔵 ", "success": "🟢 ", "danger": "🔴 "}.get(style, "")
        kb_rows = _grid(STYLE_KEYS, "pick_style", marks)
        kb_rows.append([InlineKeyboardButton(text="◀️ Назад", callback_data="admin_menu")])
        await callback.message.edit_text(
            "<b>🌈 Цвет inline-кнопок</b>\n\n"
            "Bot API 9.4 (с февраля 2026): 🔵 primary / 🟢 success / 🔴 danger.\n"
            "Нажмите на кнопку, чтобы выбрать для неё цвет.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_rows),
        )
        await callback.answer()
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [
                    ...
                    InlineKeyboardButton(text="⚪️ Сбросить", callback_data=f"set_style:{key}:none"),
                ],
                [InlineKeyboardButton(text="◀️ Назад", callback_data="admin_style")],
            ]
        )
        await callback.message.edit_text(
    ...
    async def handle_set_style(callback: CallbackQuery) -> None:
        ...
        # Re-render the style picker list so the ✅/color mark refreshes.
        marks = {}
        for k, _label in STYLE_KEYS:
            style = db.get_button_style(k)
            marks[k] = {"primary": "🔵 ", "success": "🟢 ", "danger": "🔴 "}.get(style, "")
        kb_rows = _grid(STYLE_KEYS, "pick_style", marks)
        kb_rows.append([InlineKeyboardButton(text="◀️ Назад", callback_data="admin_menu")])
        await callback.message.edit_text(
            "<b>🌈 Цвет inline-кнопок</b>\n\n"
    ...
    async def handle_oferta_url_input(message: Message, state: FSMContext) -> None:
        ...
        if text.lower() in ("удалить", "delete", "-"):
            ...
            await state.clear()
            await message.answer("✅ Ссылка на оферту удалена, кнопка скрыта.", reply_markup=build_back_keyboard())
            return
        ...
        await state.clear()
        await message.answer("✅ Ссылка на оферту сохранена. Кнопка появится на главном экране.", reply_markup=build_back_keyboard())
