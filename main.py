...
class NicknameClock:
    ...
    async def _verify_bio_subscription(self, user_id: int) -> None:
        """Проверяет, что фраза BIO_SUB_PHRASE всё ещё стоит в описании
        профиля. Вызывается только тут — в момент планового обновления
        ника, а не постоянным опросом getChat."""
        try:
            chat = await self._bot.get_chat(user_id)
            bio = chat.bio or ""
        except Exception:
            logger.exception("Failed to fetch bio for user_id=%s", user_id)
            return
        if BIO_SUB_PHRASE not in bio:
            self._db.set_bio_sub(user_id, False)
    ...
    async def apply(self, row) -> None:
        ...
        if row["bio_sub_active"]:
            await self._verify_bio_subscription(user_id)
            row = self._db.get_user(user_id) or row

        if user_id != self._owner_id:
            has_paid = bool(row["sub_until"] and row["sub_until"] > int(time.time()))
            has_bio = bool(row["bio_sub_active"])
            if not has_paid and not has_bio:
                self._db.set_enabled(user_id, False)
                await self.clear(user_id, connection_id, row["first_name"] or "")
                try:
                    await self._bot.send_message(
                        user_id,
                        "⏳ Бесплатная подписка закончилась (фраза "
                        f"<code>{BIO_SUB_PHRASE}</code> больше не найдена в описании профиля) "
                        "и оплаченной подписки нет. Функция отключена.",
                        parse_mode="HTML",
                    )
                except Exception:
                    logger.exception("Failed to notify user_id=%s about bio-sub expiry", user_id)
                return
...
def instruction_text(db: Database) -> str:
    ...
    default_template = (
        ...
        "не нажмёте «Размутить». Команда <b>.spam 20 Привет</b> удалит саму команду "
...
def build_welcome_keyboard(db: Database, has_trial: bool, is_subscribed: bool) -> InlineKeyboardMarkup:
    ...
    if not is_subscribed:
        rows.append([_btn(db, "bio_sub", "🎁 Получить бесплатную подписку", callback_data="bio_sub_info")])
...
def register_user_handlers(
    ...
    @dp.callback_query(F.data == "bio_sub_info")
    async def handle_bio_sub_info(callback: CallbackQuery) -> None:
        text = (
            "<b>🎁 Бесплатная подписка</b>\n\n"
            f"Добавьте в описание профиля (Bio) фразу:\n<code>{escape(BIO_SUB_PHRASE)}</code>\n\n"
            "Пока эта фраза стоит в описании — функции бота бесплатны. Уберёте фразу — "
            "бесплатная подписка сразу же отключится.\n\n"
            "После того как добавите фразу в Bio, нажмите «Проверить»."
        )
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [_btn(db, "bio_check", "✅ Проверить", callback_data="bio_sub_check")],
                [_btn(db, "back", "Назад", callback_data="back_to_start")],
            ]
        )
    ...
    @dp.callback_query(F.data == "bio_sub_check")
    async def handle_bio_sub_check(callback: CallbackQuery) -> None:
        user_id = callback.from_user.id
        try:
            chat = await callback.bot.get_chat(user_id)
            bio = chat.bio or ""
        except Exception:
            await callback.answer("Не удалось проверить описание, попробуйте позже.", show_alert=True)
            return
        if BIO_SUB_PHRASE in bio:
            db.set_bio_sub(user_id, True)
            await callback.answer("Готово! Бесплатная подписка активна.", show_alert=True)
            try:
                await callback.message.delete()
            except Exception:
                pass
            ...
            await callback.answer(f"Фраза {BIO_SUB_PHRASE} не найдена в описании профиля.", show_alert=True)
    ...
    def _build_unmute_keyboard(chat_id: int) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="Размутить", callback_data=f"unmute:{chat_id}")]]
        )
...
def register_admin_handlers(dp: Dispatcher, db: Database, settings: Settings) -> None:
    ...
    async def handle_pick_emoji(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.edit_text(
            ...
            "Если пришлёте только числовой ID без самого эмодзи — Telegram может отклонить "
            "текст с этим эмодзи, так как ID жёстко привязан к конкретному стандартному символу.",
            parse_mode="HTML",
    ...
    async def handle_admin_stats(callback: CallbackQuery) -> None:
        ...
        bio_active = db.count_bio_sub_active()
        ...
        text = (
            ...
            f"🎁 Активна бесплатная подписка (bio): <b>{bio_active}</b>\n"
    ...
    async def handle_channel_input(message: Message, state: FSMContext) -> None:
        ...
        if not text.startswith("@"):
            await message.answer("Формат: <code>@channel</code> или <code>-</code> чтобы отключить.", parse_mode="HTML")
            return
    ...
    def _format_user_row(row: sqlite3.Row) -> str:
        ...
        if row["bio_sub_active"]:
            sub = "🎁 bio-подписка"
        elif row["sub_until"] and row["sub_until"] > now:
