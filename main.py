...
# на доброжелательные + эмодзи). Список не претендует на исчерпывающий
# охват всех языков планеты — это практичный набор самых частых русских и
# английских грубых корней с учётом окончаний/падежей.
# ----------------------------------------------------------------------
_PROFANITY_PATTERNS = [
    r"ху[йяеёюи]\w*", r"бля[дт]?\w*", r"еб[а-я]{1,6}\b", r"сук[а-я]*\b",
    r"пизд\w*", r"муд[ао]к\w*", r"долбо[её]б\w*", r"гандон\w*", r"чмо\w*",
    r"мраз\w*", r"скотин\w*", r"тварь\w*", r"урод\w*", r"тупиц\w*",
    r"идиот\w*", r"дебил\w*", r"даун\w*", r"кретин\w*", r"дура\w*", r"дурак\w*",
    r"fuck\w*", r"shit\w*", r"bitch\w*", r"asshole\w*", r"bastard\w*",
    r"idiot\w*", r"moron\w*", r"stupid\w*",
]
...
def _contains_profanity(text: str) -> bool:
    return bool(text) and bool(_PROFANITY_RE.search(text))
...
def register_user_handlers(
    ...
    def _build_flag_off_keyboard(chat_id: int, flag: str, label: str) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text=label, callback_data=f"flagoff:{flag}:{chat_id}")]]
        )
            ...
            if text == ".clone":
                ...
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text="Режим клонирования включен.",
                        reply_markup=_build_flag_off_keyboard(chat_id, "clone", "Выключить клонирование"),
                    )
                except Exception:
            ...
            if text == ".nomat":
                ...
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text="Фильтр мата включен — сообщения с матом и оскорблениями будут удаляться.",
                        reply_markup=_build_flag_off_keyboard(chat_id, "nomat", "Выключить фильтр мата"),
                    )
                except Exception:
            ...
            if text == ".kind":
                ...
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text="Режим доброты включен — ваши грубые слова в этом чате будут заменяться на добрые.",
                        reply_markup=_build_flag_off_keyboard(chat_id, "kind", "Выключить режим доброты"),
                    )
                except Exception:
