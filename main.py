...
def _xo_board_keyboard(db: Database, chat_id: int, row: sqlite3.Row, prefix: Optional[str] = None) -> InlineKeyboardMarkup:
    """prefix=None — обычная игра в бизнес-чате (callback_data содержит
    chat_id: xo_mv:<chat_id>:<idx>, xo_new:<chat_id>).
    prefix="xof_mv" — свободная игра с ботом (без chat_id в колбэке: сам
    игрок определяется по from_user.id, поэтому cell/new данные короче:
    xof_mv:<idx>, xof_new)."""
    board = row["board"]
    status = row["status"]
    ...
    for r in range(_XO_SIZE):
        ...
        for c in range(_XO_SIZE):
            ...
            if sym == "X":
                ...
                cell_cb = f"{prefix}:{idx}" if is_free else f"xo_mv:{chat_id}:{idx}"
...
def register_user_handlers(
    ...
    # -- Крестики-нолики против бота: доступно ЛЮБОМУ пользователю бота,
    # без подключения бизнес-аккаунта и без подписки — используем тот же
    # xo_games со спец. chat_id=0, где владелец записи это сам игрок, а
    # ходы за "O" бот делает сразу же после хода игрока.
    _XOF_CHAT_ID = 0

    @dp.callback_query(F.data == "xof_new")
    async def handle_xof_new(callback: CallbackQuery) -> None:
        player_id = callback.from_user.id
        db.xo_new_game(player_id, _XOF_CHAT_ID)
        game_row = db.xo_get(player_id, _XOF_CHAT_ID)
        try:
            await callback.message.answer(
                render_emoji_tags(db, _xo_status_text(db, game_row, "Вы", "Бот")),
                parse_mode="HTML",
                reply_markup=_xo_board_keyboard(db, _XOF_CHAT_ID, game_row, prefix="xof_mv"),
            )
        except Exception:
    ...
    async def handle_xof_move(callback: CallbackQuery) -> None:
        ...
        game_row = db.xo_get(player_id, _XOF_CHAT_ID)
        if not game_row or game_row["status"] != "active" or game_row["turn"] != "X":
            await callback.answer("Игра не активна. Начните новую.", show_alert=True)
            return
        ...
        if winner:
            ...
            status = "active"
            # Ход бота сразу же, одним обновлением сообщения.
            bot_idx = _xo_bot_move(board_str, "O")
            board_str = board_str[:bot_idx] + "O" + board_str[bot_idx + 1:]
            winner = _xo_check_winner(board_str)
            if winner:
                status = f"won_{winner}"
            elif _xo_is_full(board_str):
                status = "draw"
        db.xo_save(player_id, _XOF_CHAT_ID, board_str, "X", status)
        game_row = db.xo_get(player_id, _XOF_CHAT_ID)
        try:
            await callback.message.edit_text(
                render_emoji_tags(db, _xo_status_text(db, game_row, "Вы", "Бот")),
            ...
            logger.exception("Failed to update free xo board, player_id=%s", player_id)
        await callback.answer()
