...
def build_xo_keyboard(db: Database, chat_id: int, board: str, emoji_x: str, emoji_o: str, finished: bool) -> InlineKeyboardMarkup:
    ...
    if finished:
        ...
        # Добавляем кнопку размута если есть мут
        pass
...
def register_handlers(
    ...
    async def handle_unmute_user(callback: CallbackQuery) -> None:
        ...
        muted_info = db.get_muted_info(chat_id, callback.message.chat.id)
        if not muted_info:
            await callback.answer("Нет активного мута")
            return
        
        # Размучиваем
        db.unmute_user(chat_id, callback.message.chat.id)
        await callback.message.edit_text(
            f"🔊 {callback.message.chat.first_name or 'Собеседник'} размучен",
            business_connection_id=muted_info["business_connection_id"] if muted_info else None,
        )
        await callback.answer()
    ...
    async def admin_remove_sub_start(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer(
            "Отправьте user_id пользователя, у которого отобрать подписку:"
        )
        await callback.answer()
