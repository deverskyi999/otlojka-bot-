...
TEXT_KEYS = {
    "welcome_text": (
        "👋 Привет! Это <b>TimeNick</b> — бот для Telegram Premium-бизнес-аккаунтов.\n\n"
        "Он автоматически показывает в вашей фамилии текущее время (или курс BTC/USD, "
        "USD/RUB, EUR/RUB — на выбор), чтобы собеседники видели это прямо в чате, "
...
# ---------------------------------------------------------------------------
# Курсы валют/крипты для ника
# ---------------------------------------------------------------------------

RATE_REFRESH_SECONDS = 300  # обновлять курсы не чаще раза в 5 минут
...
class RatesProvider:
    ...
    async def _maybe_refresh(self) -> None:
        now = datetime.now(timezone.utc)
        if self._last_fetch and (now - self._last_fetch).total_seconds() < RATE_REFRESH_SECONDS:
            return
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.get(
                        ...
                        self._cache["btc"] = float(data["bitcoin"]["usd"])
                    async with session.get("https://open.er-api.com/v6/latest/USD") as resp:
                        data = await resp.json()
                        rates = data.get("rates", {})
                        if "RUB" in rates:
                            self._cache["usd_rub"] = float(rates["RUB"])
                            if "EUR" in rates:
                                self._cache["eur_rub"] = float(rates["RUB"]) / float(rates["EUR"])
                self._last_fetch = now
            except Exception:
                logger.exception("Failed to refresh currency/crypto rates")


# ---------------------------------------------------------------------------
# Часы/курсы в нике (бизнес-функция бота)
# ---------------------------------------------------------------------------

class NicknameClock:
    def __init__(self, bot: Bot, db: Database, default_tz_offset_hours: float, rates: RatesProvider) -> None:
        self._bot = bot
        self._db = db
    ...
    async def _label_for_mode(self, mode: str, tz_offset_hours: float) -> str:
        if mode == "btc":
            rate = await self._rates.get("btc")
            if rate:
                return f"• ₿ {rate:,.0f}$"
        elif mode == "usd":
            rate = await self._rates.get("usd_rub")
            if rate:
                return f"• $ {rate:.2f}₽"
        elif mode == "eur":
            rate = await self._rates.get("eur_rub")
            if rate:
                return f"• € {rate:.2f}₽"
    ...
    async def apply(
        ...
        label = await self._label_for_mode(mode, tz_offset_hours)
...
def build_settings_keyboard(current_mode: str, notify_deletions: bool) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            mode_buttons[:2],
            mode_buttons[2:],
            [InlineKeyboardButton(text=notify_label, callback_data="toggle_notify_deletions")],
...
def build_settings_text(current_mode: str, tz_offset: float) -> str:
    return (
        "<b>⚙️ Настройки</b>\n\n"
        f"Формат ника: <b>{NICKNAME_MODES.get(current_mode, current_mode)}</b>\n"
        f"Часовой пояс: <b>UTC{tz_offset:+g}</b>\n"
        "Курсы обновляются не чаще раза в 5 минут.\n\n"
...
def build_subscription_required_text(db: Database) -> str:
    return db.get_text("subscription_required_text")
...
def register_handlers(
    ...
    async def handle_settings_menu(callback: CallbackQuery) -> None:
        ...
        await callback.message.edit_text(
            build_settings_text(row["nickname_mode"], tz_offset),
            parse_mode="HTML",
    ...
    async def handle_set_mode(callback: CallbackQuery) -> None:
        ...
        await callback.message.edit_text(
            build_settings_text(mode, tz_offset),
            parse_mode="HTML",
    ...
    async def handle_set_tz(callback: CallbackQuery) -> None:
        ...
        if row and row["enabled"] and row["business_connection_id"]:
            await clock.apply(
                user_id, row["business_connection_id"], row["first_name"] or "",
                row["nickname_mode"], offset,
            )
        await callback.message.edit_text(
            build_settings_text(row["nickname_mode"] if row else "time", offset),
            parse_mode="HTML",
...
async def main() -> None:
    ...
    rates = RatesProvider()
    clock = NicknameClock(bot, db, float(settings.timezone_offset_hours), rates)
    ...
    asyncio.create_task(run_update_loop(bot, db, clock, float(settings.timezone_offset_hours), settings.owner_id))
