...
_PROFANITY_PATTERNS = [
    ...
    # формы с приставкой, которые раньше ловились только нечётким поиском
    # опечаток (а значит — почти никогда для реальных слов, не опечаток)
    r"похуй\w*", r"нахер\w*", r"выблядок\w*", r"выблядк\w*", r"уебан\w*",
    r"ебанут\w*", r"ёбанут\w*", r"ёбнут\w*", r"наебал\w*", r"наебыва\w*",
]
...
async def _delete_business_message_hard(bot: "Bot", connection_id: str, message_id: int, owner_id: int) -> bool:
    ...
    получает временную ошибку (флуд-контрол / сетевой сбой — например,
    когда собеседник шлёт .spam и присылает много сообщений почти
    одновременно), повторяет попытку ещё несколько раз с нарастающей
    паузой вместо того, чтобы сразу сдаться после одной неудачи. Возвращает
    True, если сообщение реально удалено."""
    max_attempts = 4
    backoff = (0.4, 0.8, 1.5)  # секунды между попытками 1→2, 2→3, 3→4
...
async def _ai_chat_completion(
    ...
    """Запрос к ИИ через OpenAI-совместимый API. Возвращает текст ответа или
    None при ошибке/пустом ответе. base_url/api_key/model берутся из Settings
    (задаются через .env, либо используются значения по умолчанию, зашитые
    в код).

    max_tokens ограничен, чтобы ответ генерировался и приходил быстрее (для
    чата это уместно — длинные простыни текста там всё равно неудобно
    читать); при желании можно поднять в Settings.ai_max_tokens.

    system_prompt переопределяется вызывающим кодом под конкретную задачу
    (например, для .kind нужно именно перефразирование, а не ответ на
    вопрос — с другим системным промптом)."""
    try:
            ...
            "messages": [
                {
                    ...
                    "content": system_prompt or (
                        "Отвечай кратко и по делу, в чат мессенджера. Форматируй только "
                        "через Markdown (**жирный**, *курсив*, `код`, ```блоки кода```) — "
        ...
        async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
            async with session.post(f"{settings.ai_base_url}/chat/completions", json=payload) as resp:
                if resp.status != 200:
                    logger.error("AI API returned status %s: %s", resp.status, await resp.text())
                    return None
...
def _markdown_to_telegram_html(text: str) -> str:
    """Конвертирует ответ ИИ из Markdown в HTML-разметку, которую понимает
    Telegram (<b>, <i>, <code>, <pre>) — чтобы в чате не вылезали "сырые"
    звёздочки/решётки/бэктики, а форматирование реально применялось.

    Порядок важен: сначала вырезаем блоки кода и инлайн-код (защищаем их
    содержимое от дальнейшего экранирования/замен через плейсхолдеры),
    потом экранируем остальной текст под HTML, потом конвертируем
    заголовки/жирный/курсив, и в конце возвращаем код на место."""
    blocks: list[str] = []
...
async def _ai_kindify(settings: "Settings", text: str) -> Optional[str]:
    """Пытается через ИИ мягко перефразировать грубое сообщение владельца в
    доброжелательное — используется .kind ВМЕСТО простой замены слов, если
    получается прилично. Работает молча — никак не афишируется, что тут
    подключён ИИ (для собеседника это просто "смягчённая" фраза).

    Защита от вырожденных ответов: если ИИ вместо перефразирования начинает
    отвечать/пояснять/раздувать текст — результат отбраковывается, и
    вызывающий код должен откатиться на обычную замену слов (_kindify)."""
    prompt = (
        "Перефразируй эту фразу вежливо и по-доброму, сохранив её смысл и примерно "
        "такую же длину. Не отвечай на неё и не поясняй — верни ТОЛЬКО перефразированную "
        "фразу, без кавычек и подписей:\n\n" + text
    )
    system_prompt = "Ты просто перефразируешь фразы вежливее. Никогда не отвечаешь на них по смыслу, только меняешь тон."
    result = await _ai_chat_completion(settings, prompt, system_prompt=system_prompt, max_tokens=150, timeout_seconds=12)
    if not result:
        return None
    result = result.strip().strip('"').strip("«»").strip()
    if not result:
        return None
    # Похоже, что ИИ "ответил" вместо того, чтобы перефразировать — слишком
    # длинный/многострочный результат или явная вводная фраза.
    if len(result) > max(80, len(text) * 3):
        return None
    if result.count("\n") > 1:
        return None
    lowered = result.lower()
    bad_starts = (
        "конечно", "вот перефраз", "вот вежлив", "разумеется", "хорошо, вот",
        "я не могу", "извините, но", "к сожалению", "вот более",
    )
...
TEXT_EMOJI_KEYS: list[tuple[str, str]] = [
    ...
    ("promo_gift", "Кнопка: активировать промокод"),
    ("ai_icon", ".ai: иконка перед ответом"),
]
...
class Database:
    ...
    # -- стиль текста (.bold/.italic/... ) — ОДИН активный стиль на чат за
    # раз: хранится как chat_flag с префиксом "style_", но перед установкой
    # нового стиля все остальные "style_*" для этого чата снимаются.
    def set_text_style(self, owner_user_id: int, chat_id: int, style: str) -> None:
        with self.connect() as conn:
            conn.execute(
                "DELETE FROM chat_flags WHERE owner_user_id = ? AND chat_id = ? AND flag LIKE 'style_%'",
                (owner_user_id, chat_id),
            )
            conn.execute(
                "INSERT OR IGNORE INTO chat_flags (owner_user_id, chat_id, flag) VALUES (?, ?, ?)",
                (owner_user_id, chat_id, f"style_{style}"),
            )

    def clear_text_style(self, owner_user_id: int, chat_id: int) -> None:
        with self.connect() as conn:
            conn.execute(
                "DELETE FROM chat_flags WHERE owner_user_id = ? AND chat_id = ? AND flag LIKE 'style_%'",
                (owner_user_id, chat_id),
    ...
    def get_text_style(self, owner_user_id: int, chat_id: int) -> Optional[str]:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT flag FROM chat_flags WHERE owner_user_id = ? AND chat_id = ? AND flag LIKE 'style_%' LIMIT 1",
                (owner_user_id, chat_id),
            ).fetchone()
            return row["flag"][len("style_"):] if row else None
...
async def _poll_one_invoice(
    ...
    if status == "paid":
        ...
        try:
            ...
            await show_menu_screen(bot, db, settings, user_id)
        except Exception:
...
CHAT_HELP_TEXT = (
    ...
    "🤡 .troll / .untroll — бот сам отвечает на сообщения собеседника\n"
    "📨 .nomute / .unnomute — дублирует ваши сообщения от имени бота\n"
    "🎨 .bold .italic .mono .line .crossed .hidden .quote .code — стиль текста (выкл: .un + название)"
    "</blockquote>"
)
...
COMMAND_DESCRIPTIONS: dict[str, str] = {
    ...
    "ai": ".ai вопрос — задаёт вопрос ИИ прямо в чате (например: .ai что такое линейное уравнение?). Команда удаляется, бот присылает иконку (её можно сменить в админке → Премиум-эмодзи → ai_icon) и следом ответ.",
    "troll": ".troll — бот сам отвечает на каждое входящее сообщение собеседника (через ИИ): по-доброму — по-доброму, на грубость — спокойно, без мата и оскорблений. Выключить: кнопка ниже либо .untroll.",
    "nomute": ".nomute — дублирует каждое ваше сообщение ещё одной копией от имени бота (страховка на случай проблем с доставкой). Выключить: .unnomute.",
    "style": ".bold / .italic / .mono / .line / .crossed / .hidden / .quote / .code — включает стиль оформления для всех следующих ваших сообщений (жирный/курсив/моноширинный/подчёркнутый/зачёркнутый/спойлер/цитата/код). Выключить: кнопка ниже либо .un + название стиля.",
}

# -- стили текста (.bold/.italic/... ) --------------------------------
_STYLE_TAGS: dict[str, tuple[str, str]] = {
    "bold": ("<b>", "</b>"),
    "italic": ("<i>", "</i>"),
    "mono": ("<code>", "</code>"),
    "underline": ("<u>", "</u>"),
...
_STYLE_INFO: dict[str, tuple[str, str]] = {
    "bold": ("жирный", ".unbold"),
    "italic": ("курсив", ".unitalic"),
    "mono": ("моноширинный", ".unmono"),
    "underline": ("подчёркнутый", ".unline"),
    "strike": ("зачёркнутый", ".uncrossed"),
    "spoiler": ("скрытый текст (спойлер)", ".unhidden"),
    "quote": ("цитата", ".unquote"),
    "code": ("код", ".uncode"),
}

_STYLE_COMMANDS: dict[str, tuple[str, bool]] = {
    ".bold": ("bold", True), ".unbold": ("bold", False),
    ".italic": ("italic", True), ".unitalic": ("italic", False),
    ".mono": ("mono", True), ".unmono": ("mono", False),
    ".line": ("underline", True), ".unline": ("underline", False),
    ".crossed": ("strike", True), ".uncrossed": ("strike", False),
    ".hidden": ("spoiler", True), ".unhidden": ("spoiler", False),
    ".quote": ("quote", True), ".unquote": ("quote", False),
    ".code": ("code", True), ".uncode": ("code", False),
}


def _apply_text_style(style: str, text: str) -> str:
    """Оборачивает экранированный текст в HTML-тег выбранного стиля."""
    open_tag, close_tag = _STYLE_TAGS.get(style, ("", ""))
...
async def show_welcome_screen(bot: Bot, db: Database, settings: Settings, user_id: int) -> None:
    """/start шлёт РОВНО ОДНО сообщение: приветствие + 2 кнопки («Меню» и
    «Подписаться на канал»). Второй экран (не подключено / подписка
    неактивна / главное меню) показывается отдельно, только по нажатию
    «Меню» — см. show_menu_screen."""
...
async def show_menu_screen(bot: Bot, db: Database, settings: Settings, user_id: int) -> None:
    """Второй экран — показывается по кнопке «Меню» (или командой /menu):
    статус подключения / оплаты и сама панель (вкл-выкл, промокод,
    статистика, настройки, поддержка, купить подписку)."""
    row = db.get_user(user_id)
...
async def show_start_screen(bot: Bot, db: Database, settings: Settings, user_id: int) -> None:
    """Устаревшая обёртка (совместимость) — шлёт оба экрана подряд, как
    раньше. Новый код должен использовать show_welcome_screen ИЛИ
    show_menu_screen по отдельности, а не эту функцию."""
    await show_welcome_screen(bot, db, settings, user_id)
    await show_menu_screen(bot, db, settings, user_id)
...
def register_user_handlers(
    ...
    async def handle_start(message: Message, command: CommandObject) -> None:
        ...
        await show_welcome_screen(message.bot, db, settings, user_id)

    @dp.message(Command("menu"))
    async def handle_menu_cmd(message: Message) -> None:
        user_id = message.from_user.id
        db.upsert_user(user_id, message.from_user.first_name or "", message.from_user.username)
        await show_menu_screen(message.bot, db, settings, user_id)
    ...
    async def handle_open_menu_screen(callback: CallbackQuery) -> None:
        ...
        await show_menu_screen(callback.bot, db, settings, user_id)
    ...
    async def handle_check_channel(callback: CallbackQuery) -> None:
        ...
        if await passes_channel_gate(callback.bot, db, user_id):
            ...
            try:
                ...
                pass
            await show_welcome_screen(callback.bot, db, settings, user_id)
        else:
    ...
    async def handle_home_button(message: Message) -> None:
        ...
        await show_menu_screen(message.bot, db, settings, message.from_user.id)

    @dp.callback_query(F.data == "back_to_start")
    async def handle_back_to_start(callback: CallbackQuery) -> None:
        await callback.message.delete()
        await show_menu_screen(callback.bot, db, settings, callback.from_user.id)
        await callback.answer()
    ...
    async def handle_business_message(message: Message) -> None:
        ...
        if is_from_owner:
            ...
            if text == ".troll":
                try:
                    ...
                    logger.exception("Failed to delete .troll command message, chat_id=%s", chat_id)
                db.set_chat_flag(owner_id, chat_id, "troll")
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text=(
                            "Режим авто-ответчика включен: бот будет сам отвечать на входящие "
                            "сообщения (по-доброму — по-доброму, на грубость — спокойно, без мата). "
                            "Выключить: .untroll"
                        ),
                        reply_markup=_build_flag_off_keyboard(chat_id, "troll", "Выключить авто-ответчик"),
                    )
                except Exception:
                    logger.exception("Failed to send .troll notice, chat_id=%s", chat_id)
                return
            ...
            if text == ".untroll":
                try:
                    ...
                    logger.exception("Failed to delete .untroll command message, chat_id=%s", chat_id)
                db.clear_chat_flag(owner_id, chat_id, "troll")
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text="Авто-ответчик выключен.",
                    )
                except Exception:
                    logger.exception("Failed to send .untroll notice, chat_id=%s", chat_id)
                return
            ...
            if text == ".nomute":
                try:
                    ...
                    logger.exception("Failed to delete .nomute command message, chat_id=%s", chat_id)
                db.set_chat_flag(owner_id, chat_id, "nomute")
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text=(
                            "Дублирование включено: каждое ваше следующее сообщение бот сразу "
                            "продублирует ещё одной копией от своего имени (на случай проблем с "
                            "доставкой). Выключить: .unnomute"
                        ),
                        reply_markup=_build_flag_off_keyboard(chat_id, "nomute", "Выключить дублирование"),
                    )
                except Exception:
                    logger.exception("Failed to send .nomute notice, chat_id=%s", chat_id)
                return
            ...
            if text == ".unnomute":
                try:
                    ...
                    logger.exception("Failed to delete .unnomute command message, chat_id=%s", chat_id)
                db.clear_chat_flag(owner_id, chat_id, "nomute")
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text="Дублирование сообщений выключено.",
                    )
                except Exception:
                    logger.exception("Failed to send .unnomute notice, chat_id=%s", chat_id)
                return

            style_cmd = _STYLE_COMMANDS.get(text)
            if style_cmd:
                style_key, is_on = style_cmd
                try:
                    await message.bot.delete_business_messages(
                        business_connection_id=connection_id, message_ids=[message.message_id]
                    )
                except Exception:
                    logger.exception("Failed to delete style command message, chat_id=%s", chat_id)
                if is_on:
                    db.set_text_style(owner_id, chat_id, style_key)
                    label, off_cmd = _STYLE_INFO[style_key]
                    try:
                        await message.bot.send_message(
                            chat_id=chat_id,
                            business_connection_id=connection_id,
                            text=f"Стиль «{label}» включен — все следующие сообщения будут в этом стиле. Выключить: {off_cmd}",
                            reply_markup=_build_flag_off_keyboard(chat_id, f"style_{style_key}", "Выключить стиль"),
                        )
                    except Exception:
                        logger.exception("Failed to send style-on notice, chat_id=%s", chat_id)
                else:
                    db.clear_text_style(owner_id, chat_id)
                    try:
                        await message.bot.send_message(
                            chat_id=chat_id,
                            business_connection_id=connection_id,
                            text="Стиль сообщений выключен, дальше пишем обычным текстом.",
                        )
                    except Exception:
                        logger.exception("Failed to send style-off notice, chat_id=%s", chat_id)
                return
            ...
            if text.startswith(".ai"):
                ...
                try:
                    ...
                    logger.exception("Failed to delete .ai command message, chat_id=%s", chat_id)
                if not question:
                    try:
                        await message.bot.send_message(
                            chat_id=chat_id,
                            business_connection_id=connection_id,
                            text="Формат команды: .ai вопрос, например: .ai что такое линейное уравнение?",
                        )
                    except Exception:
                        logger.exception("Failed to send .ai usage hint, chat_id=%s", chat_id)
                    return
                icon_html = render_emoji_tags(db, "{emoji:ai_icon}")
                answer = await _ai_chat_completion(settings, question)
                if answer:
                    body = _markdown_to_telegram_html(answer)
                    full_text = f"{icon_html} {body}\n\n-@see_time_bot"
                else:
                    full_text = "⚠️ Не получилось получить ответ от ИИ. Попробуйте ещё раз."
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                    ...
                    logger.exception("Failed to send .ai answer, chat_id=%s", chat_id)
                return
            ...
            if db.has_chat_flag(owner_id, chat_id, "kind") and _contains_profanity(text):
                try:
                    ...
                    rephrased = await _ai_kindify(settings, text)
                    if rephrased:
                        emoji = " ".join(secrets.choice(_KIND_EMOJI) for _ in range(2))
                        final_text = f"{rephrased} {emoji}"
                    else:
                        final_text = _kindify(text)
                    try:
                        await message.bot.send_message(
                            chat_id=chat_id,
                            business_connection_id=connection_id,
            ...
            # .bold/.italic/... (стиль текста) и .nomute (дублирование) —
            # применяются к обычным сообщениям владельца, если включены.
            style = db.get_text_style(owner_id, chat_id)
            nomute_on = db.has_chat_flag(owner_id, chat_id, "nomute")
            if text and (style or nomute_on):
                if style:
                    # Стиль подменяет исходное сообщение: удаляем оригинал и
                    # шлём переоформленную HTML-версию вместо него.
                    try:
                        await message.bot.delete_business_messages(
                            business_connection_id=connection_id, message_ids=[message.message_id]
                        )
                    except Exception:
                        logger.exception("Failed to delete owner message for text style, chat_id=%s", chat_id)
                    out_text, out_parse_mode = _apply_text_style(style, text), "HTML"
                else:
                    # .nomute без стиля: оригинал остаётся как есть (не
                    # трогаем/не удаляем), а следом шлём точную копию от
                    # имени бота — на случай проблем с доставкой исходного.
                    out_text, out_parse_mode = escape(text), "HTML"
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text=out_text,
                        parse_mode=out_parse_mode,
                    )
                except Exception:
                    logger.exception("Failed to send styled/nomute message, chat_id=%s", chat_id)
            return
        ...
        if db.has_chat_flag(owner_id, chat_id, "troll") and text:
            # Авто-ответчик: реагирует на КАЖДОЕ сообщение собеседника через
            # ИИ. Специально НЕ генерирует мат/оскорбления в ответ на грубость
            # — только спокойно осаживает, без эскалации (это осознанное
            # ограничение: полностью автоматическая, неконтролируемая
            # ругань в адрес живого человека — по сути инструмент травли).
            troll_system_prompt = (
                "Ты отвечаешь вместо человека в его личной переписке, пока он отошёл. "
                "Если сообщение доброе/дружелюбное — отвечай так же тепло и по-доброму. "
                "Если сообщение грубое, с матом или оскорблениями — НЕ отвечай тем же, не "
                "материся и не оскорбляй в ответ; вместо этого спокойно и коротко осади "
                "('давай по-нормальному', 'спокойнее' и т.п.) без мата. Ответ короткий, "
                "1-2 предложения, разговорный стиль, без пояснений от себя."
            )
            reply = await _ai_chat_completion(
                settings, text, system_prompt=troll_system_prompt, max_tokens=120, timeout_seconds=15
            )
            if reply:
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text=_markdown_to_telegram_html(reply),
                        parse_mode="HTML",
                    )
                except Exception:
                    logger.exception("Failed to send .troll auto-reply, chat_id=%s", chat_id)
    ...
    _FLAG_OFF_LABELS = {
        ...
        "troll": "Авто-ответчик выключен.",
        "nomute": "Дублирование сообщений выключено.",
        **{f"style_{k}": "Стиль сообщений выключен, дальше пишем обычным текстом." for k in _STYLE_TAGS},
    }
