...
async def _ai_chat_completion(settings: "Settings", prompt: str) -> Optional[str]:
    """Запрос к ИИ через OpenAI-совместимый API (для команды .ai). Возвращает
    текст ответа или None при ошибке/пустом ответе. base_url/api_key/model
    берутся из Settings (задаются через .env, либо используются значения
    по умолчанию, зашитые в код)."""
    try:
        ...
        if is_from_owner:
            ...
            if text.startswith(".ai"):
                ...
                try:
                    ...
                    logger.exception("Failed to send .ai icon, chat_id=%s", chat_id)
                answer = await _ai_chat_completion(settings, question)
                try:
                    await message.bot.send_message(
                        chat_id=chat_id,
                        business_connection_id=connection_id,
                        text=escape(answer) if answer else "⚠️ Не получилось получить ответ от ИИ. Попробуйте ещё раз.",
                    )
                except Exception:
