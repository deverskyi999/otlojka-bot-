...
# ============================================================================
# БАЗА ДАННЫХ (сокращенная версия для экономии места)
# ============================================================================
    ...
    def init_schema(self) -> None:
        with self.connect() as conn:
            ...
            default_buttons = {
                ...
                "copy": {"label": "Скопировать юзернейм", "style": None, "callback_data": None, "row_order": 2},
                ...
                "admin_back": {"label": "⬅️ Назад", "style": None, "callback_data": "admin_home", "row_order": 99},
                "admin_stats": {"label": "📊 Статистика", "style": None, "callback_data": "admin_stats", "row_order": 10},
                "admin_users": {"label": "👥 Пользователи", "style": None, "callback_data": "admin_users", "row_order": 11},
                "admin_buttons": {"label": "🎨 Управление кнопками", "style": None, "callback_data": "admin_buttons", "row_order": 12},
                "admin_give_sub": {"label": "⭐ Выдать подписку", "style": "success", "callback_data": "admin_give_sub", "row_order": 13},
                "admin_mutes": {"label": "🔇 Управление мутами", "style": "danger", "callback_data": "admin_mutes", "row_order": 14}
            }
...
def register_handlers(dp: Dispatcher, db: Database, clock: NicknameClock, 
    ...
    # ==================== КОМАНДА /admin (ОСНОВНАЯ) ====================
    
    @dp.message(Command("admin"))
    async def handle_admin_command(message: Message) -> None:
        """Главная команда для входа в админ-панель"""
        ...
        if not is_admin(user_id):
            await message.answer(
                ...
                "Если вы администратор, добавьте этот ID в переменную ADMIN_IDS.",
                parse_mode="HTML"
            )
            return
    ...
    async def check_admin(message: Message) -> None:
        ...
        await message.answer(
            ...
            f"Если вы админ, но бот говорит что нет - "
            f"проверьте переменную ADMIN_IDS в настройках хостинга.",
            parse_mode="HTML"
        )
    ...
    async def handle_start(message: Message) -> None:
        ...
        if not is_subscribed:
            await message.answer(
                ...
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="⭐ Оплатить подписку (25 ⭐)", callback_data="pay_subscription")]
                ])
            )
            return
        ...
        if not is_connected:
            await message.answer(
                ...
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    ...
                    [InlineKeyboardButton(text="📋 Скопировать юзернейм", 
        ...
        buttons = []
        if is_enabled:
            buttons.append([InlineKeyboardButton(text="🔴 Выключить время", callback_data="toggle_off")])
        else:
            buttons.append([InlineKeyboardButton(text="🟢 Включить время", callback_data="toggle_on")])
        buttons.append([InlineKeyboardButton(text="ℹ️ Помощь по муту", callback_data="mute_help")])
    ...
    async def handle_toggle_on(callback: CallbackQuery) -> None:
        ...
        await callback.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔴 Выключить время", callback_data="toggle_off")],
                [InlineKeyboardButton(text="ℹ️ Помощь по муту", callback_data="mute_help")]
            ])
        )
        await callback.answer()
    ...
    async def handle_toggle_off(callback: CallbackQuery) -> None:
        ...
        await callback.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🟢 Включить время", callback_data="toggle_on")],
                [InlineKeyboardButton(text="ℹ️ Помощь по муту", callback_data="mute_help")]
            ])
        )
        await callback.answer()
