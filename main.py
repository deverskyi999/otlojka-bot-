...
WELCOME_TEXT_DEFAULT = (
    ...
    "/spam &lt;число&gt; &lt;текст&gt; - 💬 Спам сообщений\n"
    "/xo - 🎮 Крестики-нолики с собеседником\n"
    "/auto - 🤖 Настройка автоответа"
)
...
class Database:
        ...
        with self.connect() as conn:
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    xo_board TEXT,
                    xo_current_player TEXT,
                    xo_opponent_id INTEGER
                )
                """
            )
            conn.execute(
            ...
            for col, ddl in (
                ...
                ("is_muted", "ALTER TABLE users ADD COLUMN is_muted INTEGER NOT NULL DEFAULT 0"),
                ("auto_reply_enabled", "ALTER TABLE users ADD COLUMN auto_reply_enabled INTEGER NOT NULL DEFAULT 0"),
                ("auto_reply_text", "ALTER TABLE users ADD COLUMN auto_reply_text TEXT"),
                ("xo_board", "ALTER TABLE users ADD COLUMN xo_board TEXT"),
                ("xo_current_player", "ALTER TABLE users ADD COLUMN xo_current_player TEXT"),
                ("xo_opponent_id", "ALTER TABLE users ADD COLUMN xo_opponent_id INTEGER"),
            ):
    ...
    def get_admins(self) -> list[sqlite3.Row]:
        with self.connect() as conn:
            return conn.execute("SELECT * FROM users WHERE is_admin = 1 ORDER BY started_at DESC").fetchall()
    ...
    def set_muted(self, user_id: int, muted: bool) -> None:
        with self.connect() as conn:
            conn.execute("UPDATE users SET is_muted = ? WHERE user_id = ?", (int(muted), user_id))

    def is_muted(self, user_id: int) -> bool:
        row = self.get_user(user_id)
        return bool(row and row["is_muted"])

    def set_auto_reply(self, user_id: int, enabled: bool, text: Optional[str] = None) -> None:
        with self.connect() as conn:
            conn.execute(
                "UPDATE users SET auto_reply_enabled = ?, auto_reply_text = ? WHERE user_id = ?",
                (int(enabled), text, user_id),
            )

    def get_auto_reply(self, user_id: int) -> tuple[bool, Optional[str]]:
        row = self.get_user(user_id)
        if not row:
            return False, None
        return bool(row["auto_reply_enabled"]), row["auto_reply_text"]

    def save_xo_game(self, user_id: int, board: list, current_player: str, opponent_id: int) -> None:
        with self.connect() as conn:
            conn.execute(
                "UPDATE users SET xo_board = ?, xo_current_player = ?, xo_opponent_id = ? WHERE user_id = ?",
                (json.dumps(board), current_player, opponent_id, user_id),
            )

    def get_xo_game(self, user_id: int) -> tuple[Optional[list], Optional[str], Optional[int]]:
        row = self.get_user(user_id)
        if not row or not row["xo_board"]:
            return None, None, None
        try:
            board = json.loads(row["xo_board"])
            return board, row["xo_current_player"], row["xo_opponent_id"]
        except:
            return None, None, None

    def clear_xo_game(self, user_id: int) -> None:
        with self.connect() as conn:
            conn.execute(
                "UPDATE users SET xo_board = NULL, xo_current_player = NULL, xo_opponent_id = NULL WHERE user_id = ?",
                (user_id,),
            )
    ...
    async def _handle_permission_loss(self, user_id: int) -> None:
        ...
        try:
            await self._bot.send_message(
                user_id,
                "Недостаточно прав для смены фамилии. Переподключите бота в настройках, разрешив изменение имени.",
            )
        except Exception:
...
def build_settings_text(current_mode: str, tz_offset: float, target_datetime: Optional[str] = None, countdown_label: Optional[str] = None) -> str:
    ...
    lines.append("\n🔔 Уведомления об удалении: бот пришлёт вам сам факт, что собеседник удалил сообщение в переписке, без содержимого удалённого текста.")
    return "\n".join(lines)
...
def build_admin_buttons_keyboard(db: Database) -> InlineKeyboardMarkup:
    ...
    for row in db.get_all_buttons():
        rows.append([InlineKeyboardButton(text=f"{row['label']}", callback_data=f"admin_btn:{row['button_key']}")])
    rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="admin_home")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def register_handlers(dp: Dispatcher, db: Database, clock: NicknameClock, bot_username: str, settings: Settings) -> None:
    ...
    async def handle_start(message: Message) -> None:
        ...
        if is_new:
            ...
            if row and not row["trial_used"]:
                ...
                await message.answer("🎁 Вам начислен бесплатный пробный период на 3 дня.", parse_mode="HTML")
        ...
        await message.answer(status_text, parse_mode="HTML", reply_markup=build_toggle_keyboard(db, is_enabled))
    ...
    async def handle_successful_payment(message: Message) -> None:
        ...
        await message.answer("<b>Подписка активирована ✅</b>\nВам доступны все функции бота на 30 дней.", parse_mode="HTML")
        row = db.get_user(user_id)
        is_connected = bool(row and row["business_connection_id"])
        if not is_connected:
            await message.answer(build_not_connected_text(db), parse_mode="HTML", reply_markup=build_connect_keyboard(db, bot_username))
    ...
    async def handle_business_connection(connection: BusinessConnection) -> None:
        ...
        if connection.is_enabled:
            ...
            if not db.is_subscribed(user_id, settings.owner_id):
                ...
                return
            try:
                await connection.bot.send_message(user_id, build_connected_text(db), parse_mode="HTML", reply_markup=build_toggle_keyboard(db, False))
            except Exception:
    ...
    async def handle_toggle_on(callback: CallbackQuery) -> None:
        ...
        await callback.message.edit_text("<b>Время в нике включено.</b>", parse_mode="HTML", reply_markup=build_toggle_keyboard(db, True))
        await callback.answer()
    ...
    async def handle_toggle_off(callback: CallbackQuery) -> None:
        ...
        if row["business_connection_id"]:
            await clock.clear(user_id, row["business_connection_id"], row["first_name"] or "")
        await callback.message.edit_text("<b>Время в нике выключено.</b>", parse_mode="HTML", reply_markup=build_toggle_keyboard(db, False))
        await callback.answer()
    ...
    async def handle_settings_back(callback: CallbackQuery) -> None:
        ...
        status_text = "<b>Время в нике включено.</b>" if is_enabled else "<b>Время в нике выключено.</b>"
        await callback.message.edit_text(status_text, parse_mode="HTML", reply_markup=build_toggle_keyboard(db, is_enabled))
        await callback.answer()
    ...
    async def handle_tz_menu(callback: CallbackQuery) -> None:
        await callback.message.edit_text(
            ...
            "Можно выбрать кнопкой ниже или ввести вручную числом (например <code>+3</code>, <code>-5.5</code>) или названием города/страны (например «Москва», «London», «Дубай»).",
            parse_mode="HTML",
    ...
    async def handle_tz_manual_start(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer(
            "Введите часовой пояс числом (например <code>+3</code>) или названием города/страны (например «Алматы»):",
            parse_mode="HTML",
        )
        await callback.answer()
    ...
    async def handle_tz_manual_finish(message: Message, state: FSMContext) -> None:
        ...
        await state.clear()
        if offset is None:
            await message.answer("Не удалось распознать часовой пояс. Попробуйте число от -12 до +14 (например -5.5) или известное название города.")
            return
        db.set_timezone(user_id, offset)
        row = db.get_user(user_id)
        if row and row["enabled"] and row["business_connection_id"]:
    ...
    async def handle_set_target_date_start(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer(
            ...
            "Форматы: <code>31.12.2026</code>, <code>31.12.2026 20:00</code>, <code>2026-12-31 20:00</code>.\n"
    ...
    async def handle_set_target_date_finish(message: Message, state: FSMContext) -> None:
        ...
        if naive_dt is None:
            await message.answer("Не удалось распознать дату. Попробуйте, например: <code>31.12.2026 20:00 До Нового года</code>", parse_mode="HTML")
            return
        row = db.get_user(user_id)
        tz_offset = db.get_timezone(row, settings.timezone_offset_hours)
    ...
    async def _start_feedback(user_id: int, first_name: str, username: Optional[str], state: FSMContext, answer_func) -> None:
        ...
        await answer_func(
            "✉️ Напишите одним сообщением, что хотите передать поддержке. Чтобы отменить, нажмите «Отмена» или отправьте /cancel.",
            reply_markup=InlineKeyboardMarkup(
    ...
    async def handle_feedback_finish(message: Message, state: FSMContext) -> None:
        ...
        forward_text = f"📩 <b>Обратная связь</b>\nОт: {who} (<code>{sender.id}</code>)\n\n{message.text}"
        try:
    ...
    async def handle_admin(message: Message) -> None:
        ...
        if not is_admin(user_id):
            return
        await message.answer("<b>Админ-панель</b>", parse_mode="HTML", reply_markup=build_admin_main_keyboard(is_owner(user_id)))
    ...
    async def admin_home(callback: CallbackQuery) -> None:
        if not is_admin(callback.from_user.id):
            await callback.answer()
            return
        await callback.message.edit_text("<b>Админ-панель</b>", parse_mode="HTML", reply_markup=build_admin_main_keyboard(is_owner(callback.from_user.id)))
        await callback.answer()
    ...
    async def admin_texts_menu(callback: CallbackQuery) -> None:
        if not is_admin(callback.from_user.id):
            await callback.answer()
            return
        await callback.message.edit_text("<b>📝 Тексты бота</b>\nВыберите, что изменить:", parse_mode="HTML", reply_markup=build_admin_texts_keyboard())
        await callback.answer()
    ...
    async def admin_settext_start(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer("Отправьте новый текст (можно с HTML-разметкой: &lt;b&gt;, &lt;i&gt;, &lt;code&gt;):")
        await callback.answer()
    ...
    async def admin_set_feedback_target_start(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer("Отправьте user_id или @username получателя обратной связи.\n\n⚠️ Если укажете @username, бот сможет доставлять сообщения только если этот человек уже хотя бы раз запускал бота (/start). Telegram не позволяет писать по username в обход этого.")
        await callback.answer()
    ...
    async def admin_set_feedback_target_finish(message: Message, state: FSMContext) -> None:
        ...
        if target_id is not None:
            ...
            db.set_setting("feedback_target_username", target_row["username"] if target_row and target_row["username"] else None)
            await message.answer(f"✅ Обратная связь теперь идёт на <code>{target_id}</code>.", parse_mode="HTML")
            return
        username = raw.lstrip("@")
        with db.connect() as conn:
            found = conn.execute("SELECT user_id FROM users WHERE username = ?", (username,)).fetchone()
        if found:
            ...
            db.set_setting("feedback_target_username", username)
            await message.answer(f"Пользователь @{username} ещё не запускал этого бота. Сохранил username, но доставка заработает только после того, как он хоть раз нажмёт /start.")
    ...
    async def admin_buttons(callback: CallbackQuery) -> None:
        if not is_admin(callback.from_user.id):
            await callback.answer()
            return
        await callback.message.edit_text("<b>🎨 Настройка кнопок</b>\nВыберите кнопку:", parse_mode="HTML", reply_markup=build_admin_buttons_keyboard(db))
        await callback.answer()
    ...
    def _button_edit_text(key: str) -> str:
        ...
        return (
            ...
            f"<i>Premium emoji на кнопке отображается только если у владельца бота есть активная подписка Telegram Premium.</i>"
        )
    ...
    async def admin_set_label_finish(message: Message, state: FSMContext) -> None:
        ...
        await state.clear()
        await message.answer(f"Текст кнопки «{key}» обновлён.", parse_mode="HTML", reply_markup=build_admin_button_edit_keyboard(key))
    ...
    async def admin_set_emoji_start(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer("Отправьте сюда сообщение с premium-эмодзи (просто отправьте стикер-эмодзи как обычное сообщение), я возьму его custom_emoji_id автоматически.\n\nРаботает только если у владельца бота есть Telegram Premium.")
        await callback.answer()
    ...
    async def admin_set_emoji_finish(message: Message, state: FSMContext) -> None:
        ...
        if not emoji_id and message.text:
            ...
            if candidate.isdigit():
                emoji_id = candidate
        if not emoji_id:
            await message.answer("Не нашёл premium-эмодзи в сообщении. Отправьте сообщение, содержащее именно premium-эмодзи, либо пришлите его числовой ID.")
            return
        db.set_button_emoji(key, emoji_id)
        await state.clear()
        await message.answer(f"Premium emoji для кнопки «{key}» установлен.", parse_mode="HTML", reply_markup=build_admin_button_edit_keyboard(key))
    ...
    async def admin_grant_sub_start(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer("Отправьте user_id пользователя, которому выдать подписку на 30 дней:")
        await callback.answer()
    ...
    async def admin_grant_sub_finish(message: Message, state: FSMContext) -> None:
        ...
        try:
            await message.bot.send_message(target_id, "🎁 Вам выдана подписка на 30 дней администратором бота.")
        except Exception:
    ...
    async def admin_price_menu(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.edit_text(
            f"<b>💰 Цена подписки</b>\n\nТекущая цена: <b>{db.get_price()} ⭐</b> за 30 дней.\n\nОтправьте новое число Stars (например <code>25</code>), чтобы изменить.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="admin_home")]]
            ),
    ...
    async def admin_price_finish(message: Message, state: FSMContext) -> None:
        ...
        await message.answer(f"✅ Цена подписки обновлена: {db.get_price()} ⭐ за 30 дней.", parse_mode="HTML", reply_markup=build_admin_main_keyboard(is_owner(message.from_user.id)))
    ...
    async def admin_admins_menu(callback: CallbackQuery) -> None:
        if not is_owner(callback.from_user.id):
            ...
            return
        await callback.message.edit_text("<b>🛡 Управление админами</b>", parse_mode="HTML", reply_markup=build_admin_admins_keyboard())
        await callback.answer()
    ...
    async def admin_list_admins(callback: CallbackQuery) -> None:
        ...
        if not admins:
            ...
            text = "\n".join(lines)
        await callback.message.edit_text(text, parse_mode="HTML", reply_markup=build_admin_admins_keyboard())
        await callback.answer()
    ...
    async def admin_grant_admin_start(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer("Отправьте user_id пользователя, которого сделать админом.\nОн получит доступ к статистике, юзерам, кнопкам и выдаче подписок (без права назначать других админов).")
        await callback.answer()
    ...
    async def admin_grant_admin_finish(message: Message, state: FSMContext) -> None:
        ...
        try:
            await message.bot.send_message(target_id, "🛡 Вам выдан доступ к админ-панели бота. Откройте её командой /admin")
        except Exception:
    ...
    # ========== НОВЫЕ КОМАНДЫ (ПРАВИЛЬНЫЕ) ==========

    # 1. /mute - Замутить собеседника
    ...
    async def handle_mute(message: Message) -> None:
        ...
        if not row or not row["business_connection_id"]:
            await message.answer("❌ Бот не подключён к бизнес-чату.")
            return
        
        if not db.is_subscribed(user_id, settings.owner_id):
            await message.answer("❌ Требуется подписка.")
            return
        
        chat_id = message.chat.id
        if chat_id == user_id:
            await message.answer("❌ Используйте /mute в диалоге с собеседником.")
            return
        
        opponent_id = chat_id
        db.set_muted(opponent_id, True)
        
        try:
            await message.bot.send_message(
                opponent_id,
                "🔇 Вы замучены собеседником. Ваши сообщения не будут видны.\nДля размута нажмите кнопку ниже.",
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[[
                        InlineKeyboardButton(text="🔊 Размутить", callback_data=f"unmute:{user_id}")
                    ]]
                )
            )
        except Exception:
        ...
        await message.answer("✅ Собеседник замучен. Его сообщения будут скрыты.")

    @dp.callback_query(F.data.startswith("unmute:"))
    async def handle_unmute(callback: CallbackQuery) -> None:
        target_id = int(callback.data.split(":")[1])
        db.set_muted(target_id, False)
        await callback.message.edit_text("🔊 Вы размучены.")
        await callback.answer("Размут выполнен.")

    # 2. Перехват сообщений замученных
    @dp.message(F.text)
    async def handle_muted_messages(message: Message) -> None:
        ...
        if db.is_muted(user_id):
            try:
                await message.bot.send_message(
                    settings.owner_id,
                    f"🚫 Замученный пользователь {user_id} попытался отправить:\n{message.text}"
                )
            except Exception:
                pass
            ...
            await message.answer("🤫 Вы замучены. Сообщение не отправлено.")
            return

    # 3. /spam - Спам
    @dp.message(Command("spam"))
    async def handle_spam(message: Message, command: CommandObject) -> None:
        user_id = message.from_user.id
        ...
        await message.answer(f"✅ Начинаю спам из {count} сообщений...")
        
        for i in range(count):
            await asyncio.sleep(0.5)
            try:
                await message.bot.send_message(chat_id, f"{i+1}. {spam_text}")
            except Exception as e:
                await message.answer(f"❌ Ошибка при спаме: {e}")
                break

    # 4. /xo - Крестики-нолики
    @dp.message(Command("xo"))
    async def handle_xo(message: Message, command: CommandObject) -> None:
        user_id = message.from_user.id
        ...
        args = command.args
        opponent_username = args.lstrip("@") if args else None
        
        if opponent_username:
            with db.connect() as conn:
                opponent = conn.execute(
                    "SELECT user_id FROM users WHERE username = ?", (opponent_username,)
                ).fetchone()
            if not opponent:
                await message.answer("❌ Пользователь не найден. Он должен запустить бота /start.")
                return
            opponent_id = opponent["user_id"]
        else:
            row = db.get_user(user_id)
            if not row or not row["business_connection_id"]:
                await message.answer("❌ Бот не подключён. Используйте /xo @username чтобы вызвать конкретного пользователя.")
                return
        ...
        db.save_xo_game(user_id, board, "X", opponent_id)
        db.save_xo_game(opponent_id, board, "O", user_id)
        
        await send_xo_board(message.bot, user_id, opponent_id, board, "X")
        await send_xo_board(message.bot, opponent_id, user_id, board, "O")
        await message.answer("✅ Игра началась! Ваш ход.")

    async def send_xo_board(bot: Bot, user_id: int, opponent_id: int, board: list, current_player: str) -> None:
        board_text = ""
        for row in board:
            board_text += "|".join(row) + "\n"
            board_text += "-" * 5 + "\n"
            ...
            for j in range(3):
                if board[i][j] == " ":
                    row_buttons.append(
                        InlineKeyboardButton(
                            text="⬜",
                            callback_data=f"xo_move:{i}:{j}:{opponent_id}"
                        )
                    )
                else:
                    row_buttons.append(
        ...
        keyboard.append([
            InlineKeyboardButton(text="❌ Завершить игру", callback_data="xo_end")
        ])
    ...
    def check_xo_winner(board: list) -> Optional[str]:
        for i in range(3):
    ...
    @dp.callback_query(F.data.startswith("xo_move:"))
    async def handle_xo_move(callback: CallbackQuery) -> None:
        _, i, j, opponent_id = callback.data.split(":")
        i, j = int(i), int(j)
        opponent_id = int(opponent_id)
        user_id = callback.from_user.id
        
        board, current_player, saved_opponent = db.get_xo_game(user_id)
        if not board:
            await callback.answer("❌ Игра не найдена.")
            return
        
        if saved_opponent != opponent_id:
        ...
        if winner:
            await callback.message.edit_text(f"🏆 {winner} победил!")
            db.clear_xo_game(user_id)
            db.clear_xo_game(opponent_id)
        ...
        db.save_xo_game(user_id, board, next_player, opponent_id)
        db.save_xo_game(opponent_id, board, next_player, user_id)
        
        await send_xo_board(callback.bot, user_id, opponent_id, board, next_player)
        await send_xo_board(callback.bot, opponent_id, user_id, board, next_player)
        await callback.answer("Ход сделан.")

    @dp.callback_query(F.data == "xo_end")
    async def handle_xo_end(callback: CallbackQuery) -> None:
        user_id = callback.from_user.id
        db.clear_xo_game(user_id)
    ...
    async def handle_auto_reply(message: Message, command: CommandObject) -> None:
        user_id = message.from_user.id
        ...
        if not args:
            enabled, text = db.get_auto_reply(user_id)
            status = "✅ Включен" if enabled else "❌ Выключен"
            reply_text = text or "Я сейчас занят(а), отвечу позже! 📱"
            await message.answer(
                f"📝 <b>Автоответ</b>\n"
                f"Статус: {status}\n"
                f"Текст: {reply_text}\n\n"
                ...
                f"/auto set <текст> - установить текст",
                parse_mode="HTML"
            )
            return
        ...
        if action == "on":
            enabled, text = db.get_auto_reply(user_id)
            db.set_auto_reply(user_id, True, text or "Я сейчас занят(а), отвечу позже! 📱")
            await message.answer("✅ Автоответ включен.")
        elif action == "off":
            db.set_auto_reply(user_id, False, None)
            await message.answer("❌ Автоответ выключен.")
        elif action == "set" and len(parts) > 1:
            new_text = parts[1]
            db.set_auto_reply(user_id, True, new_text)
            await message.answer(f"✅ Текст автоответа установлен:\n{new_text}")
        else:
            await message.answer("❌ Неверная команда. Используйте /auto on/off/set <текст>")

    # 6. Автоответ для новых собеседников
    @dp.message(F.text)
    async def handle_auto_reply_new_user(message: Message) -> None:
        user_id = message.from_user.id
        chat_id = message.chat.id
        
        row = db.get_user(chat_id)
        if row and row["business_connection_id"]:
            owner_row = db.get_user_by_connection(row["business_connection_id"])
            if owner_row:
                enabled, reply_text = db.get_auto_reply(owner_row["user_id"])
                if enabled and reply_text:
                    try:
                        await message.bot.send_message(
                            user_id,
                            f"🤖 <b>Автоответ:</b>\n{reply_text}",
        ...
        try:
            for row in db.get_users_needing_expiry_warning(EXPIRY_WARNING_SECONDS):
                ...
                try:
                    await bot.send_message(
                        row["user_id"],
                        "⏳ Ваша подписка заканчивается меньше чем через 2 дня. Продлите её командой /start, чтобы не потерять доступ.",
                    )
                except Exception:
...
async def main() -> None:
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="mute", description="🔇 Замутить собеседника"),
        BotCommand(command="spam", description="💬 Спам сообщений собеседнику"),
        BotCommand(command="xo", description="🎮 Крестики-нолики с собеседником"),
        BotCommand(command="auto", description="🤖 Настройка автоответа"),
    ])
