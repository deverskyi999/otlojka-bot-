...
class NicknameClock:
    def __init__(self, bot: Bot, db: Database, tz_offset_hours: int, rates: RatesProvider) -> None:
        self._bot = bot
        self._db = db
    ...
    async def _label_for_mode(self, mode: str) -> str:
        if mode == "btc":
    ...
    async def apply(self, user_id: int, connection_id: str, first_name: str, mode: str = "time") -> None:
        if not connection_id:
            return

        label = await self._label_for_mode(mode)
...
def build_settings_text(current_mode: str) -> str:
    return (
        "<b>⚙️ Настройки</b>\n\n"
...
def register_handlers(
    ...
    async def handle_toggle_on(callback: CallbackQuery) -> None:
        ...
        await clock.apply(user_id, row["business_connection_id"], row["first_name"] or "", row["nickname_mode"])
    ...
    async def handle_settings_menu(callback: CallbackQuery) -> None:
        ...
        if not row:
            ...
            return
        await callback.message.edit_text(
            build_settings_text(row["nickname_mode"]),
            parse_mode="HTML",
    ...
    async def handle_set_mode(callback: CallbackQuery) -> None:
        ...
        if row and row["enabled"] and row["business_connection_id"]:
            await clock.apply(user_id, row["business_connection_id"], row["first_name"] or "", mode)
        await callback.message.edit_text(
            build_settings_text(mode),
            parse_mode="HTML",
    ...
    while True:
        ...
        for row in db.get_enabled_users():
            ...
            if not db.is_subscribed(user_id, owner_id):
                ...
                continue
            await clock.apply(
                user_id, row["business_connection_id"], row["first_name"] or "", row["nickname_mode"]
            )
...
async def main() -> None:
    ...
    rates = RatesProvider()
    clock = NicknameClock(bot, db, settings.timezone_offset_hours, rates)
